...
# ✅ Convertir nombres completos a abreviaciones para que Plotly pinte el mapa
sales_by_state["State_Code"] = sales_by_state["State"].map(state_abbrev)
 
fig_map = px.choropleth(
    sales_by_state,
    locations="State_Code",               # ✅ Abreviaciones (CA, TX, NY...)
    locationmode="USA-states",
    color="Sales",
    hover_name="State",                   # ✅ Al pasar el cursor muestra nombre completo
    color_continuous_scale="Reds",        # 🔴 Rojo intenso = más ventas
    range_color=(0, sales_by_state["Sales"].max()),
